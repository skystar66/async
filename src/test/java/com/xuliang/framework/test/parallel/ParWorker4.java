...
//            throw new RuntimeException("故意打出异常");
    ...
    public void call(boolean success, String param, WorkResult<String> workResult) {
        if (success) {
            System.out.println("callback worker4 success--" + workResult.getResult()
                    + "-threadName:" + Thread.currentThread().getName());
        } else {
            System.err.println("callback worker4 failure--" + workResult.getResult() + "-- ex " + workResult.getEx()
                    + "-threadName:" + Thread.currentThread().getName());
        }
    }
